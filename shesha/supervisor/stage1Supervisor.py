## @package   shesha.supervisor.stage1Supervisor
## @brief     Initialization and execution of a first stage supervisor
...
class Stage1Supervisor(CompassSupervisor):
    ...
    def next(self, *, move_atmos: bool = True, nControl: int = 0,
             ...
             compute_tar_psf: bool = True, do_centroids: bool = True) -> None:
        """Iterates the AO loop, with optional parameters, considering it is a first 
        stage. 

        Overload the CompassSupervisor next() method to arrange tasks orders and allow cascaded simulation.
        ...
        Kwargs:
            ...
            do_centroids : (bool) : If True (default), the last WFS image is stacked and centroids computation is done. WFS image is next reset.
        """
        try:
            iter(nControl)
        except TypeError:
        ...
        if ( geo_index > -1):
            ...
            if self.rtc is not None:
                for ncontrol in nControl : # range(len(self.config.p_controllers)):                                                          
                    ...
                    # Also udeful for calibration. (CBE 2023.01.30)
                    if do_centroids:
