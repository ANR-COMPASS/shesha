...
class RTCSupervisor(BenchSupervisor):
    ...
    def initConfig(self) -> None:
        ...
        print("->dm")
        self.fakedms = Octopus.getInterface(**self.config.p_dms[0]._actuInterface)

        print("->RTC")
        nact = self.config.p_dms[0].nact
        nvalid = p_wfs._nvalid
        self.cmat = Octopus.getInterface(**self.config.p_controllers[0]._cmatInterface)
        ...
        if p_wfs.type == WFSType.SH:
            ...
            self.config.p_nvalid = tmp_valid
            ...
            yvalid = tmp_valid[1, :]
            offset = 0
            scale = self.config.p_centroiders[0].pyrscale
        else:
            raise RuntimeError("WFS Type not usable")
        ...
        gain = self.config.p_controllers[0].gain
        print("gain : %f" % gain)
        self.rtc = rtc_standalone(self.context, wfsNb, [nvalid], nact,
                                  self.config.p_centroiders[0].type, 1, offset, scale,
                                  brahma=self.BRAHMA)
